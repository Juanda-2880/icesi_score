...
def lambda_handler(event, context):
    ...
    period_id = (event.get("pathParameters") or {}).get("id", "").strip()
    if not period_id:
    ...
    try:
        ...
        with conn.cursor() as cur:
            cur.execute(
                "SELECT id_match, period_label, end_time FROM match_period WHERE id = %s",
                (period_id,),
            )
            period_row = cur.fetchone()
        if period_row is None:
            return _response(404, {"error": "Período no encontrado"})
        if period_row[2] is not None:
            return _response(409, {"error": "El período ya está cerrado"})

        match_id = str(period_row[0])
        ...
        return _response(200, {"periodId": period_id, "endTime": end_time.isoformat()})

    except psycopg2.OperationalError as exc:
        print(f"[put_match_period] ERROR conexión: {exc}")
        return _response(500, {"error": "Error de conexión a la base de datos"})
    except psycopg2.Error as exc:
        if conn:
            conn.rollback()
        print(f"[put_match_period] ERROR SQL [{exc.pgcode}]: {exc.pgerror or exc}")
