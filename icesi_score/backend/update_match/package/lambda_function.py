...
def handler(event, _context):
    ...
    try:
        ...
        if row is None or row[0] not in ("ADMIN", "SUPERADMIN"):
            return _response(403, {"error": "No tienes permisos para editar partidos."})
        ...
        # --- Update match (only if status == SCHEDULED) ---
        with conn.cursor() as cur:
            cur.execute(
                """
                UPDATE match
        ...
        return _response(200, {"id": str(updated[0])})
        ...
        print(f"[update_match] ERROR de conexión: {exc}")
        return _response(500, {"error": "Error de conexión a la base de datos"})
    except psycopg2.Error as exc:
        print(f"[update_match] ERROR SQL [{exc.pgcode}]: {exc.pgerror or exc}")
